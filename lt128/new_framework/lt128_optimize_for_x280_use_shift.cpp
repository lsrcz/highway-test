...
#define INSTANTIATE(T)                                                         \
  ...
  Lt128OptimizeForX280UseShift<T, true, true>(T ar, T br, MFromV<T> & m);      \
  template __attribute__((noinline)) size_t                                    \
  Lt128OptimizeForX280UseShift<T, false, true>(T ar, T br, MFromV<T> & m);     \
  template __attribute__((noinline)) size_t                                    \
  Lt128OptimizeForX280UseShift<T, true, false>(T ar, T br, MFromV<T> & m);     \
  template __attribute__((noinline)) size_t                                    \
  Lt128OptimizeForX280UseShift<T, false, false>(T ar, T br, MFromV<T> & m);
