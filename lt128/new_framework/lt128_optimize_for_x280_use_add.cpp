...
__attribute__((noinline)) size_t Lt128OptimizeForX280UseAdd(Vec a, Vec b,
  ...
  LLVM_MCA_BEGIN_WITH_REP(kReplicate, "Lt128OptimizeForX280UseAdd",
  ...
  LLVM_MCA_END_WITH_REP(kReplicate, "Lt128OptimizeForX280UseAdd", ("+vr"(r)),
                        (), decltype(ar));
  GET_CYCLE(("+vr"(r)), (), end);
  m = r;
  return end - begin;
...
#define INSTANTIATE(T)                                                         \
  ...
  Lt128OptimizeForX280UseAdd<T, true, true>(T ar, T br, MFromV<T> & m);      \
  template __attribute__((noinline)) size_t                                    \
  Lt128OptimizeForX280UseAdd<T, false, true>(T ar, T br, MFromV<T> & m);     \
  template __attribute__((noinline)) size_t                                    \
  Lt128OptimizeForX280UseAdd<T, true, false>(T ar, T br, MFromV<T> & m);     \
  template __attribute__((noinline)) size_t                                    \
  Lt128OptimizeForX280UseAdd<T, false, false>(T ar, T br, MFromV<T> & m);
