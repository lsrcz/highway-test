...
__attribute__((noinline)) size_t Lt128Synthesized(Vec a, Vec b,
                                                  MFromV<Vec> &m) {
  ...
  LLVM_MCA_BEGIN_WITH_REP(kReplicate, "Lt128Synthesized",
  ...
  LLVM_MCA_END_WITH_REP(kReplicate, "Lt128Synthesized", ("+vr"(r)), (),
                        decltype(ar));
  GET_CYCLE(("+vr"(r)), (), end);
  m = r;
  return end - begin;
...
#define INSTANTIATE(T)                                                         \
  template __attribute__((noinline)) size_t Lt128Synthesized<T, true>(         \
      T ar, T br, MFromV<T> & m);                                              \
  template __attribute__((noinline)) size_t Lt128Synthesized<T, false>(        \
      T ar, T br, MFromV<T> & m);
